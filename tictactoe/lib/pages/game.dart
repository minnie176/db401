...
class _GameState extends State<Game> {
   bool _done = false;
 String _symbol = '_'; //กำหนดเป็น X หรือ เป็น O //
 String _status = 'C'; // C: challenge, P: playing, D: draw, O: O win, X: X win
 bool _isTurn = false; //เป็นการสลับ ผลัดกันคนละตา//
 List<List<String>> _filled = [
   ['_', '_', '_'], //ถว 1-2-3 ว่างๆคือ XO//
   ['_', '_', '_'],
   ['_', '_', '_'],
 ];
 late DocumentReference _game; 
  ...
  Color _gameBackground() { //เป้นการเปลี่ยนสีพื้นหลัง แพ้ ชนะ คนละสี //
   if(_status == 'D') {
  ...
  bool _winByRow() {
   for (var row in _filled) { //แพ้ชนะดูจากfilled//
     var players = <String>{...row}; 
     if (players.length == 1 && players.first == _symbol) { //สมาชิกplayers //
       return true;
     }
   }
   return false;
  ...
  bool _winByDiagonal() { //ตรวจสอบผลแบบเส้นทะแยงมุม//
  ...
  void _checkResult(int row, int col) { //เช็คผลลัพทุกครั้งที่มีการติ๊กใหม่// 
   if(!_done && _isTurn) { 
     setState(() {
       _isTurn = false;
       _filled[row][col] = _symbol;
       String filledString = _filled[0].join('') + _filled[1].join('') + _filled[2].join(''); //จับ XO ที่วางเรียงกันมาเชื่อมกัน//
       if(_winByRow() || _winByColumn() || _winByDiagonal()) { //ถ้าตัวใดตัวหนึ่งเป็นจริง ให้ ตัวนั้นชนะเลย winByRow() || _winByColumn() || _winByDiagonal())//
         _game.update({'filled': filledString, 'status': _symbol});
       } else {
         if(filledString.contains('_')) {
           _game.update({'filled': filledString, 'turn': _symbol == 'O' ? 'X' : 'O'}); //ถ้าเราเป็น โอ เปลี่ยนเป็นฝ่ายตรงข้ามคือ เอ็ก//
         } else {
 ...
 void initState() {
  ...
  builder: (context, snapshot) { //รับข้อมูลเเล้วให้ทำหน้าที่อะไรต่ออยู่ตรง builder//
    if (!snapshot.hasData) {
      return const Center(child: CircularProgressIndicator());
    }
    DocumentSnapshot data = snapshot.requireData;
    _isTurn = data['turn'] == _symbol; //turn เป็นตาเรา ถ้าไม่ช้จะเป็นฝั่งตรงข้าเล่น//
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          Row( //เอามาเรียงกันตามแนวนอน ให้อยู่ตรงกลางตามแนวนอน ด้านล่างเป็นสมาชิก//
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
